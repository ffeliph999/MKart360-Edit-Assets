...
#else
#define X360_REMAINING_BANK_ADDRESS(address) &_other_texturesSegmentRomStart[((u32) address) & 0xFFFFFF]
#endif
#include <ultra64.h>
#include <macros.h>
#include <memory.h>
...
u8* dma_copy_base_misc_textures(u8* devAddr, u8* baseAddress, u32 size, u32 offset) {
    ...
    osPiStartDma(&gDmaIoMesg, 0, 0, (uintptr_t) X360_REMAINING_BANK_ADDRESS(devAddr), address,
                 size, &gDmaMesgQueue);
