...
/* B19.4R8V3 NOTE VOLUME TRACE
 * Diagnostic only: no audio values are modified. */
static unsigned char x360_r8v3_watch[64];
static unsigned x360_r8v3_logs;
#endif
        ...
        if (playbackState->priority != NOTE_PRIORITY_DISABLED) {
...
#ifdef XBOX360_PORT
            x360_r8v3_pre_velocity = velocity;
#endif
            ...
            if ((unsigned)i < 64U) {
                if (noteSubEu->needsInit &&
                    noteSubEu->resamplingRateFixedPoint != 0 &&
                    noteSubEu->targetVolLeft == 0 &&
                    noteSubEu->targetVolRight == 0) {
                    x360_r8v3_watch[i] = 6;
                }

                if (x360_r8v3_watch[i] && x360_r8v3_logs < 400U) {
                    char x360_r8v3_msg[384];
                    sprintf(x360_r8v3_msg,
                        "MK64: B19.4R8V3 NOTE i=%d watch=%u enabled=%u init=%u finished=%u "
                        "scale=%.7f preVel=%.7f finalVel=%.7f pan=%u reverb=%u "
                        "targetLR=%u/%u rate=%04X\n",
                        i,
                        (unsigned)x360_r8v3_watch[i],
                        (unsigned)noteSubEu->enabled,
                        (unsigned)noteSubEu->needsInit,
                        (unsigned)noteSubEu->finished,
                        (double)scale,
                        (double)x360_r8v3_pre_velocity,
                        (double)velocity,
                        (unsigned)pan,
                        (unsigned)reverbVol,
                        (unsigned)noteSubEu->targetVolLeft,
                        (unsigned)noteSubEu->targetVolRight,
                        (unsigned)noteSubEu->resamplingRateFixedPoint);
                    x360_log(x360_r8v3_msg);
                    ++x360_r8v3_logs;
                    --x360_r8v3_watch[i];
                }
            }
