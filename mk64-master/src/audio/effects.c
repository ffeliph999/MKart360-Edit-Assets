...
#ifdef XBOX360_PORT
/*
 * B19.4R9 XBOX NATIVE-ENDIAN ADSR
 *
 * Keep this correction local to AdsrEnvelope reads. Xbox 360 PowerPC and
 * Nintendo 64 are both big-endian, so these in-memory 16-bit envelope fields
 * should be consumed in native order on Xbox.
 */
#define X360_ADSR16(v) (v)
#else
#define X360_ADSR16(v) BSWAP16(v)
#endif
...
f32 adsr_update(struct AdsrState* adsr) {
    ...
    switch (state) {
        ...
        case ADSR_STATE_LOOP:
            adsr->delay = X360_ADSR16(adsr->envelope[adsr->envIndex].delay);
            switch (adsr->delay) {
                case ADSR_DISABLE:
                    adsr->state = ADSR_STATE_DISABLED;
                    break;
                case ADSR_HANG:
                    ...
                    break;
                case ADSR_GOTO:
                    adsr->envIndex = X360_ADSR16(adsr->envelope[adsr->envIndex].arg);
                    goto restart;
                case ADSR_RESTART:
                ...
                default:
                    ...
                    adsr->target = (f32) X360_ADSR16(adsr->envelope[adsr->envIndex].arg) / 32767.0f;
