/* Runtime adaptation of the existing local tools/libtkmk00.c. */
#include <stdint.h>
#include <string.h>


#define DIM(x) (sizeof(x)/sizeof((x)[0]))
#define read_u16_be(p) (((unsigned int)(p)[0]<<8)|(p)[1])
#define read_u32_be(p) (((unsigned int)(p)[0]<<24)|((unsigned int)(p)[1]<<16)|((unsigned int)(p)[2]<<8)|(p)[3])
#define write_u16_be(p,v) do {(p)[0]=(uint8_t)((v)>>8);(p)[1]=(uint8_t)(v);}while(0)
...
extern "C" void tkmk00decode(uint8_t *tkmk, uint8_t *tmp_buf, uint8_t *rgba16, int32_t alpha_color)  // 800405D0/0411D0
{
   unsigned offset;
   unsigned test_bits;
   int width, height;
