...
extern "C" int x360_load_local_rom(void) {
    ...
    if(f==INVALID_HANDLE_VALUE){x360_log("MK64: existing US baserom.us.z64 must be beside the XEX\n");return 0;}
    ...
    if((crc^0xFFFFFFFFU)!=X360_EXPECTED_ROM_CRC){x360_log("MK64: unsupported or corrupt local ROM\n");return 0;}
    romReady=true;x360_log("MK64: local US ROM validated\n");return 1;
}
